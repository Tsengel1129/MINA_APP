...
// import 'package:hello_world/newScreen.dart';
// import 'package:expensemanager/reminder/util/notification_manager.dart';
import 'package:expensemanager/reminder/widgets/task.dart';
import 'package:expensemanager/reminder/widgets/weekday_picker.dart';
...
class _AddTaskState extends State<AddTask> {
  ...
  DateTime picked;
  String
      _currentTaskSubTextNoTime; //Only used locally, text containing days, dates, numbers but not any time components. This is combined with time in updateSubText();

  List<bool>
      _enabledDays; //Store the array of bools for weekday picker's enabled days
  int _repeatEveryNumber; //Number holding the input for the 'Repeat Every' text box, used for number based reminders
  DateTime
      _whenToRepeat; //Hold the DateTime object for reminders on specific days, only for 'Date' use
  ...
  int _weekdayChipIndex =
      0; //Identifier for chips letting us know weekday has been selected
  ...
  Color _mainColor = Colors.blue[900];
  ...
  Future<Null> _selectDate(BuildContext context, int remindType) async {
    this.picked = await showDatePicker(
      context: context,
    ...
    if (this.picked != null)
      setState(() {
        if (remindType == _numberChipIndex) {
          ...
          _repeatStartDate = this
              .picked
              .toLocal(); //Update _repeatStartDate when date picker is called to schedule a start date
        }
        if (remindType == _specificDayChipIndex) {
          ...
          _whenToRepeat = this
              .picked
              .toLocal(); //Update _whenToRepeat for specific date reminders
          _currentTaskSubTextNoTime = this.picked.toString().split(' ')[0];
          _currentDate = this.picked.toString().split(' ')[0];
  ...
  DateTime getReminderTime() {
    if (_choiceChipValue == 0) {
      ...
      return DateTime(DateTime.now().year, DateTime.now().month,
          DateTime.now().day, _timeFromPicker.hour, _timeFromPicker.minute);
    } else if (_choiceChipValue == 1) {
      //For number based, return _repeatStartDate + _timeFromPicker
      return DateTime(_repeatStartDate.year, _repeatStartDate.month,
          _repeatStartDate.day, _timeFromPicker.hour, _timeFromPicker.minute);
    } else if (_choiceChipValue == 2) {
      //For date based, return _whenToRepeat + _timeFromPicker
      return DateTime(_whenToRepeat.year, _whenToRepeat.month,
          _whenToRepeat.day, _timeFromPicker.hour, _timeFromPicker.minute);
    } else {
      return null;
    }
  }
  ...
  void updateSubText() {
    setState(() {
      if (_timeFromPicker.hour == 0 && _timeFromPicker.minute == 1) {
        ...
        _currentTaskSubText = _currentTaskSubTextNoTime +
            " " +
            _timeFromPicker.format(context) +
            S.of(context).reminderMinute;

        _currentTime = _timeFromPicker.format(context);
      }
    });
  }
  ...
  void getReminders() async {
    ...
    print({"myKeys": sp.getString(reminderKey)});

    if (sp.getString(reminderKey) != null) {
      json
          .decode(sp.getString(reminderKey))
        ...
        children: <Widget>[
          ...
          Expanded(
            child: TextField(
              ...
              style:
                  TextStyle(fontSize: 10.0, height: .9, color: Colors.black87),
              decoration: InputDecoration(
                labelText: S.of(context).reminderBottomSheetTextRepeatDay2,
              ...
              onChanged: (String s) {
                ...
                setState(() {
                  if (s == '') {
                    ...
                    _currentTaskSubTextNoTime = "Өдөр бүр";
                    updateSubText();
                  } else {
                    _currentTaskSubTextNoTime = s + " хоног болгон";
                    updateSubText();
                  }
                  _repeatEveryIcon = _previewCardColor;
                });
                _repeatEveryNumber = int.parse(
                    s); //Convert our trimmed string to an int to save for our Reminder object
              },
              onSubmitted: (s) {
                setState(() {
      ...
      return Container(
        child: Row(
          ...
          children: <Widget>[
            ...
            RaisedButton(
              color: _previewCardColor,
              child: Text(S.of(context).reminderBottomSheetchooseDate,
  ...
  Widget dateText() {
    ...
    return Padding(
      padding: EdgeInsets.only(right: 10.0),
      child: Text(S.of(context).reminderSelectedDate +
          this.picked.toString().split(' ')[0]),
    );
  }

  @override
  void initState() {
    _currentTaskName = 'Таны бичвэр';
    picker = new WeekdayPicker();
    _currentTaskSubText = '';
    _currentTaskSubTextNoTime = '';
    _previewCardColor = Theme.of(context).accentColor; //The default theme color
    _previewCardAccent = Colors
        .white10; //Set to transparent white so InkWells for all taps look normal
    _choiceChipValue =
        0; //Set to 0, so that the weekday chip is enabled by default
    ...
    _reminderTime = DateTime(
        ...
        DateTime.now().month,
        DateTime.now()
            .day); //Initialize reminder time with reminder time at very start of the day
    _timeFromPicker =
        TimeOfDay.now(); //Default to 12AM as our input from the time picker
            ...
            children: <Widget>[
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    labelText: S.of(context).reminderBottomSheetTextTittle,
                    fillColor: Colors.grey[100],
                    filled: true,
                    border: UnderlineInputBorder(
                      borderSide: BorderSide(
              ...
              Row(
                ...
                children: <Widget>[
                  ...
                  new ChoiceChip(
                    //Weekday Chip
                    label: Text(S.of(context).reminderBottomSheetTextRepeat),
                    selected: _choiceChipValue == _weekdayChipIndex,
                    onSelected: (bool selected) {
                      setState(() {
                        _currentTaskSubText =
                            ''; //Wipe Repeat text on each click
                        _choiceChipValue = selected
                            ? _weekdayChipIndex
                            : null; //Set as current active button, using its unique chip index
                      });
                    },
                  ),
                  new Padding(
                  ...
                  new ChoiceChip(
                    //Number Chip
                    label: Text(S.of(context).reminderBottomSheetTextRepeatDay),
                  ...
                  new ChoiceChip(
                    //Date Chip
                    label: Text(S.of(context).reminderBottomSheetTextDate),
                    selected: _choiceChipValue == _specificDayChipIndex,
                    onSelected: (bool selected) {
                      setState(() {
                        _currentTaskSubText = '';
                        _choiceChipValue =
                            selected ? _specificDayChipIndex : null;
                      });
                    },
                  ),
                ],
                  ...
                  Row(
                    children: [
                      Padding(
                        padding: EdgeInsets.only(right: 10.0),
                        child: Text(
                            S.of(context).reminderSelectedTime + _currentTime),
                      ),
                      RaisedButton(
                        color: _previewCardColor,
                        child: Text(
                          S.of(context).reminderSelectTime,
                ...
                child: Padding(
                  ...
                  child: Center(
                    child: RaisedButton(
                      ...
                      child: Text(
                        S.of(context).reminderSave,
                        style: TextStyle(color: Colors.white),
                      ),
                      onPressed: !(_currentTaskSubTextNoTime.length >
                              1) //If button subtext exists, so does valid input, thus enable the button if true
                          ? null
                          : () async {
                              ...
                              var uniqueID = (DateTime.now()
                                      ...
                                      1514786400000) ~/
                                  100; //Remove first two and last two digits from unix timestamp so digit fits inside a Java int and so we count by tenths of a second
                              ...
                              var reminder = Reminder(
                                  ...
                                  uniqueID);
                              reminders.add(
                                  reminder); //Add new reminder to current list of reminders
                              generateNotification(
                                  reminder); //Set a notification based on this reminder
