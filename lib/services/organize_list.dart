...
List<Reminder> organizeByColor(List<Reminder> currentList) {
  ...
  if (currentList.length == 0) {
    //If we receive an empty array return the same thing
    return sortedReminders;
  }
  ...
  for (int i = 0; i < currentList.length; i++) {
    ...
    while (currentIndex < sortedReminders.length && !addedReminder) {
      ...
      if (reminderInSorted.cardColor == currentReminder.cardColor) {
        ...
        addedReminder = true;
      } else if (currentIndex == sortedReminders.length - 1) {
        //Looped through all the elements but still wasn't able to find a match. Just add to the end of the array as a result
        sortedReminders.add(currentReminder);
        addedReminder = true;
      }
