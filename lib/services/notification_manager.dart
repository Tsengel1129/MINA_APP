...
final int repeatNotifCount =
    50; //Number of times to schedule a recurring notification for number based reminders
final String warningText =
    'This is the last instance of this notification, recreate it if you would like it to continue';

final int weekdayID =
    0; //Identifier for reminders that are weekday based - 'Weekday Choice Chip'
final int numberID =
    1; //Identifier for reminders that are number based - 'Number Choice Chip'
final int dateID =
    2; // Identifier for reminders that are specific day based - 'Date Choice Chip'
...
void generateNotification(Reminder reminder) async {
  ...
  if (reminderType == numberID) {
    ...
    if (repeatEvery != 0 && repeatEvery > 0) {
      ...
      repeatStartDate =
          reminderTime; //Reminder time contains both repeat start date and specified start time,
...
void rescheduleNotification(Reminder remind) {
  ...
  if (remindType == weekdayID) {
    generateNotification(
        remind); //Reschedule weekday reminders by default, won't cause repeat notif bug
    ...
    for (int i = 0; i < repeatNotifCount + 1; i++) {
      ...
      } else if (repeatStartDate.isAfter(DateTime.now()) &&
          i == repeatNotifCount - 1) {
        flutterLocalNotificationsPlugin.schedule(
            notificationID,
            remind.cardTitle,
      ...
      notificationID++;
      repeatStartDate = repeatStartDate.add(Duration(
          days:
              repeatEvery)); //Increment after so that even the first initial reminder is included
    }
